...
# Abrir a base de dados onde as referências estão guardadas

def load_references():
    conn = sqlite3.connect('references.db')
    c = conn.cursor()
    c.execute('''
        SELECT r.ID, r.Título, r.[Tipo de publicação], r.Ano, r.[Local de Publicação], r.[Número de páginas], r.DOI
        FROM Referências r
    ''')
    references = []
    for row in c.fetchall():
        ref_id = row[0]
        # Buscar autores associados à referência
        c.execute('''
            SELECT a.[Nome próprio], a.Apelido
            FROM Autores a
            JOIN ReferenciaAutores ra ON a.ID = ra.autor_id
            WHERE ra.referencia_id = ?
        ''', (ref_id,))
        authors = [{'Nome próprio': a[0], 'Apelido': a[1]} for a in c.fetchall()]
        # Buscar palavras-chave associadas à referência
        c.execute('''
            SELECT p.Palavra
            FROM PalavrasChave p
            JOIN ReferenciaPalavrasChave rp ON p.ID = rp.palavra_id
            WHERE rp.referencia_id = ?
        ''', (ref_id,))
        keywords = [k[0] for k in c.fetchall()]
        references.append({
            'Título': [row[1]],
            'Tipo de publicação': [row[2]],
            'Autores': authors,
            'Ano': [str(row[3])],
            'Local de publicação': [row[4]],
            'Número de páginas': [str(row[5])],
            'DOI': [row[6]],
...
# Função para guardar as referências para a base de dados quando o utilizador sair da aplicação

def save_on_exit():
    conn = sqlite3.connect('references.db')
    c = conn.cursor()
    # Limpar as tabelas para evitar problemas com o autoincrement
    c.execute('DELETE FROM ReferenciaPalavrasChave')
    c.execute('DELETE FROM ReferenciaAutores')
    c.execute('DELETE FROM Referências')
    c.execute('DELETE FROM Autores')
    c.execute('DELETE FROM PalavrasChave')
    for ref in references:
        # Inserir referência
        c.execute('''
            INSERT INTO Referências (Título, [Tipo de publicação], Ano, [Local de Publicação], [Número de páginas], DOI)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            ref['Título'][0],
            ref['Tipo de publicação'][0],
            int(ref['Ano'][0]) if ref['Ano'][0] else None,
            ref['Local de publicação'][0],
            int(ref['Número de páginas'][0]) if ref['Número de páginas'][0] else None,
            ref['DOI'][0]
        ))
        ref_id = c.lastrowid
        # Inserir autores associados à referência
        for author in ref['Autores']:
            c.execute('SELECT ID FROM Autores WHERE [Nome próprio]=? AND Apelido=?', (author['Nome próprio'], author['Apelido']))
            result = c.fetchone()
            if result:
                author_id = result[0]
            else:
                c.execute('INSERT INTO Autores ([Nome próprio], Apelido) VALUES (?, ?)', (author['Nome próprio'], author['Apelido']))
                author_id = c.lastrowid
            c.execute('INSERT INTO ReferenciaAutores (referencia_id, autor_id) VALUES (?, ?)', (ref_id, author_id))
        # Inserir palavras-chave associadas à referência
        for palavra in ref['Palavras-chave']:
            c.execute('SELECT ID FROM PalavrasChave WHERE Palavra=?', (palavra,))
            result = c.fetchone()
            if result:
                palavra_id = result[0]
            else:
                c.execute('INSERT INTO PalavrasChave (Palavra) VALUES (?)', (palavra,))
                palavra_id = c.lastrowid
            c.execute('INSERT INTO ReferenciaPalavrasChave (referencia_id, palavra_id) VALUES (?, ?)', (ref_id, palavra_id))
    conn.commit()
    conn.close()
    window.destroy()
